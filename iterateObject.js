...
/**
 * Intent: Iterate keys of an object.
 *
 * Utility: Iterate the keys of an AST node or transformation options.
 */
...
function forInStatement(object) {
    for (let key in object) { object[key]; };
}

/* ---- Tests -------------------------------------------------------------- */

const testOriginal = (object) => { return () => forEachKey(object) };
const testNew = (object) => { return () => forInStatement(object) };
